-- Simple SQL script to add position table and update users/department tables
...
ALTER TABLE `users` 
ADD COLUMN IF NOT EXISTS `positionID` int DEFAULT NULL AFTER `role`,
ADD COLUMN IF NOT EXISTS `phone` varchar(20) COLLATE utf8mb4_general_ci DEFAULT NULL AFTER `suffix`,
ADD COLUMN IF NOT EXISTS `profilePicture` varchar(255) COLLATE utf8mb4_general_ci DEFAULT NULL AFTER `phone`;

-- Add column to department table if it doesn't exist
ALTER TABLE `department` 
ADD COLUMN IF NOT EXISTS `positionID` int DEFAULT NULL AFTER `departmentHead`;

-- Enable foreign key checks before adding constraints
SET FOREIGN_KEY_CHECKS=1;

-- Add foreign key constraints (drop first if exists)
ALTER TABLE `users` DROP FOREIGN KEY IF EXISTS `fk_users_position`;
ALTER TABLE `users` 
...
ALTER TABLE `department` DROP FOREIGN KEY IF EXISTS `fk_department_position`;
ALTER TABLE `department` 
