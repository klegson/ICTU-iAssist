-- Add position table and update existing tables
-- Run this in phpMyAdmin or via mysql command
...
-- Add positionID column to users table (if not exists)
ALTER TABLE `users` 
ADD COLUMN `positionID` int DEFAULT NULL AFTER `role`,
ADD COLUMN `phone` varchar(20) COLLATE utf8mb4_general_ci DEFAULT NULL AFTER `suffix`,
ADD COLUMN `profilePicture` varchar(255) COLLATE utf8mb4_general_ci DEFAULT NULL AFTER `phone`;

-- Add positionID column to department table (if not exists)
ALTER TABLE `department` 
ADD COLUMN `positionID` int DEFAULT NULL AFTER `departmentHead`;
